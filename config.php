...
/**
 * DirectAdmin Registration Script - Configuration
 * --------------------------------------------------
 * Adjust these values to match your DirectAdmin server.
 */

// ── DirectAdmin Server ────────────────────────────────────────────────────────
define('DA_HOST', 'https://your-server.example.com'); // e.g. https://demo.directadmin.com
define('DA_PORT', 2222);                               // Integer: default DA port

// ── SSL Certificate Verification ─────────────────────────────────────────────
// Set to true in production (recommended).
// Set to false ONLY if your DA server uses a self-signed certificate.
define('DA_SSL_VERIFY', true);
...
//
// Recommendation: Use a Login Key with limited scope instead of the main password.
// Create one under DirectAdmin → Your Account → Login Keys.
// 
// IMPORTANT FOR LOGIN KEYS:
// 1. DA_ADMIN_USER must be your DirectAdmin username (e.g. 'admin'). Do NOT append the key name.
// 2. DA_ADMIN_PASS must be the Login Key secret generated by DirectAdmin (e.g. 'your-key-secret').
// 
...
define('DA_ADMIN_USER', 'admin');
define('DA_ADMIN_PASS', 'your-admin-password-or-login-key-secret');

// ── Default Package for New Users ──────────────────────────────────────────
// Must be an existing reseller package on the DA server.
define('DA_DEFAULT_PACKAGE', 'default');

// ── IP Address for New Accounts ──────────────────────────────────────────────
// Enter the main IP address assigned to your Reseller account in DirectAdmin (e.g. '123.45.67.89').
// Note: If 'shared' gives "A valid IP was not provided", replace 'shared' with your actual server IP.
define('DA_IP', 'shared');

// ── Send notification email to the new user? ───────────────────────────────
// NOTE: Setting this to 'yes' may expose your server as a spam source.
define('DA_NOTIFY_USER', 'no'); // 'yes' or 'no'
...
// ── Password Policy ─────────────────────────────────────────────────────────
// Minimum password length for new accounts (DirectAdmin recommends at least 8).
...
// ── Site Title & Branding ───────────────────────────────────────────────────
define('SITE_TITLE', 'DirectAdmin – Registration');
define('PANEL_URL', DA_HOST . ':' . DA_PORT . '/');

// ── Card Heading & Subheading ────────────────────────────────────────────────
// The large heading and the small uppercase subtitle shown on the registration card.
define('CARD_HEADING', 'DirectAdmin');
